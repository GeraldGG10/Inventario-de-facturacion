# Guía de Instalación y Distribución: Tecno-laser
...
- Empaquetará todo en el archivo **`Tecno-laser.exe`**.
...
- `Tecno-laser.exe` (El programa principal)
...
La máquina principal es el computador donde estará guardada la base de datos y donde se ejecutará el `Tecno-laser.exe`. 

### A. Instalar PostgreSQL
Como Tecno-laser usa PostgreSQL, debes instalarlo en la máquina principal antes de correr el sistema por primera vez:
...
3. **¡IMPORTANTE!** Durante la instalación, te pedirá que crees una contraseña para el superusuario `postgres`. 
   - Debes poner la contraseña que está configurada en tu archivo `.env`. Por defecto es: `tecnolaser2026`.
...
2. Conéctate con la contraseña (`tecnolaser2026`).
3. Haz clic derecho en "Databases" -> Create -> Database.
4. Nombra la base de datos como **`tecnolaser`** y guárdala.
...
¡Buenas noticias! Tecno-laser está diseñado para funcionar sin que tengas que configurar la red de forma manual. 
...
1. Hacer **doble clic en `Tecno-laser.exe`**.
...
6. Los empleados solo deben abrir ese enlace en sus navegadores y verán la pantalla de login de Tecno-laser. *(Recuerda: todos deben estar conectados a la misma red Wi-Fi o cableada).*
