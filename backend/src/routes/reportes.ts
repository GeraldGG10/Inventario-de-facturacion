...
reportesRouter.get('/orden-compra', async (_req, res) => {
  ...
  // LOGO / TÍTULO
  doc.font('Helvetica-Bold').fontSize(20).fillColor(primaryColor).text('ORDEN DE COMPRA SUGERIDA', 50, 50);
  doc.font('Helvetica').fontSize(10).fillColor('#6B7280')
    .text(`Generada el ${new Date().toLocaleString('es-DO')}`, 50, 75);
  doc.text(`Empresa: ${nombreEmpresa}`, 50, 90);

  // TABLA
  let y = 140;
  doc.rect(50, y, 495, 22).fillColor(tableHeaderBg).fill();
  doc.font('Helvetica-Bold').fontSize(9).fillColor(primaryColor);
  doc.text('CÓDIGO', 55, y + 7, { width: 70 });
  doc.text('PRODUCTO', 130, y + 7, { width: 170 });
  doc.text('PROVEEDOR', 305, y + 7, { width: 110 });
  doc.text('ACTUAL', 420, y + 7, { width: 50, align: 'center' });
  doc.text('PEDIR', 475, y + 7, { width: 50, align: 'center' });
  ...
  for (const p of productos) {
    ...
    const pedir = Math.max(p.stockMinimo * 2 - p.stockActual, p.stockMinimo);
    doc.text(p.codigo, 55, y, { width: 70 });
    doc.text(p.nombre, 130, y, { width: 168 });
    doc.text(p.proveedor?.nombre ?? 'Sin proveedor', 305, y, { width: 108 });
    doc.fillColor('#DC2626').text(p.stockActual.toString(), 420, y, { width: 50, align: 'center' });
    doc.fillColor('#059669').font('Helvetica-Bold').text(pedir.toString(), 475, y, { width: 50, align: 'center' });
    doc.font('Helvetica').fillColor(primaryColor);
    
    y += 16;
    doc.moveTo(50, y - 2).lineTo(545, y - 2).lineWidth(0.3).strokeColor('#E5E7EB').stroke();
  }
  ...
  for (let i = 0; i < pages.count; i++) {
    doc.switchToPage(i);
    doc.font('Helvetica').fontSize(8).fillColor('#9CA3AF');
    doc.text(`Página ${i + 1} de ${pages.count}`, 50, 750, { align: 'center', width: 495 });
  }
