...
async function seedUsuarioAdmin() {
    ...
    if (!existente) {
        ...
        const passwordHash = await bcryptjs_1.default.hash('CambiarEsta123!', 10);
...
async function seedCatalogoDemo() {
    ...
    if (categoriasExistentes > 0)
        return;
    const categorias = await Promise.all(['Electrónica', 'Mobiliario', 'Suministros', 'Materiales de Construcción'].map((nombre) => prisma.categoria.create({ data: { nombre } })));
