...
async function main() {
    ...
    const categorias = await Promise.all(categoriasData.map(c => prisma.categoria.create({ data: c })));
    // ============================================================
    // UBICACIONES
    // ============================================================
    const ubicacionesData = ['Pasillo A', 'Pasillo B', 'Pasillo C', 'Refrigeración', 'Congelados', 'Bodega'];
    const ubicaciones = await Promise.all(ubicacionesData.map(nombre => prisma.ubicacion.create({ data: { nombre } })));
    // ============================================================
    // PROVEEDORES (30)
    // ============================================================
    ...
    const proveedores = await Promise.all(proveedoresData.map(p => prisma.proveedor.create({ data: { ...p, condicionesPago: 'credito_30' } })));
    // ============================================================
    ...
    const productos = await Promise.all(productosData.map(p => prisma.producto.create({
        data: {
            codigo: p.codigo,
            nombre: p.nombre,
    ...
    async function crearFactura(clienteIdx, productosIdxConCantidad, diasAtras, hora, metodoPago, ref) {
    ...
    for (const f of facturasACrear) {
        await crearFactura(f.cli, [...f.prods], f.dias, f.hora, f.mp, f.ref);
    }
